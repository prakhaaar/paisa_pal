            ...
            child: ListView(
              children: [
                ...
                Padding(
                  ...
                  child: Save(
                    formKey: this.type == 'Income' ? _formKey4 : _formKey5,
                  ...
                  onTap: () async {
                    IconData? selectedIcon =
                        await Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => SelectIcon(widget.type),
                          ),
                        ) ??
                        selectedCategoryIcon;

                    context.read<ChangeCategory>().changeCategoryIcon(
                      selectedIcon,
                    );
                  },
                  child: Column(
  ...
  Widget build(BuildContext context) {
    void saveCategoryFunction() {
      if (widget.formKey.currentState!.validate()) {
        ...
        IconData? finalCategoryIcon = Provider.of<ChangeCategory>(
          context,
          listen: false,
        ).selectedCategoryIcon;
        ...
        void updateCategory() {
          Provider.of<ChangeExpenseItemEdit>(
            widget.contextExEdit!,
            listen: false,
          ).getAllExpenseItems();
          Provider.of<ChangeExpenseItem>(
            widget.contextEx!,
            listen: false,
          ).getAllExpenseItems();
        }
        ...
        if (widget.contextInEdit != null) {
          ...
          if (widget.categoryName != null) {
            ...
            if (finalCategoryName != widget.categoryName ||
              ...
              incomeItems.removeWhere(
                (item) => item.text == widget.categoryName,
              );
            }
          }
          incomeItems.add(
          ...
          Provider.of<ChangeIncomeItemEdit>(
            widget.contextInEdit!,
            listen: false,
          ).getIncomeItems();
          if (widget.contextIn != null) {
            Provider.of<ChangeIncomeItem>(
              widget.contextIn!,
              listen: false,
            ).getIncomeItems();
          }
          ...
          if ((widget.categoryName == null) && (widget.parentItem == null)) {
            CategoryItem item = categoryItem(
              finalCategoryIcon ?? Icons.category_outlined,
            );
            print('add expense');
            if (finalParent!.text ==
                getTranslated(context, 'Parent category')!) {
            ...
            if (widget.parentItem == null) {
              ...
              if (finalCategoryName != widget.categoryName ||
                  ...
                  widget.description != finalDescription) {
                print('something changed');
                List<CategoryItem> items = sharedPrefs.getItems(
                  widget.categoryName!,
                );
                items.removeAt(0);
                items.insert(
    ...
    if (widget.categoryName == null && widget.parentItem == null) {
      ...
      return SaveAndDeleteButton(
        ...
        parentExpenseItem: widget.parentItem == null
            ? null
            : widget.parentItem!.text,
